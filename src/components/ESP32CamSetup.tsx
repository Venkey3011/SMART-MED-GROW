...
export default function ESP32CamSetup({ device, token, onDeviceUpdated, userHistory }: Props) {
  ...
  const protectedPath = (path: string) => `${path}${path.includes('?') ? '&' : '?'}key=${encodeURIComponent(device?.authToken || '')}`;
  ...
  const request = async <T,>(path: string): Promise<T> => {
    ...
    try { const response = await fetch(`${baseUrl}${protectedPath(path)}`, { cache: 'no-store', signal: controller.signal }); if (!response.ok) throw new Error(`ESP32-CAM returned ${response.status}`); return response.json() as Promise<T>; } finally { window.clearTimeout(timeout); }
  };
  ...
  const discoverThroughHotspot = async () => {
    ...
    try {
      const response = await fetch(`http://${HOTSPOT_IP}${protectedPath('/api/info')}`, { cache: 'no-store' }); if (!response.ok) throw new Error('ESP32-CAM hotspot did not respond.');
  ...
  const capture = () => setLastPhoto(`${baseUrl}${protectedPath(`/capture?t=${Date.now()}`)}`);

  return <div className="space-y-6">
    <section className="bg-white border border-zinc-100 rounded-xl p-5 shadow-sm"><h3 className="text-zinc-800 font-bold text-lg flex items-center gap-2"><Radio className="w-5 h-5 text-emerald-600" /> ESP32-CAM connection</h3><p className="mt-2 text-xs text-zinc-500 leading-relaxed">The camera remains powered down by default. It turns on only when an authorized user opens the live view or captures a photo, then turns off again when that view closes.</p>{device && <p className="mt-3 text-[11px] font-mono text-zinc-500">Device: {device.deviceId}</p>}</section>
